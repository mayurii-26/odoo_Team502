...
class RecommendationEngine:
    ...
    def _load_weights(self) -> Dict[str, Dict[str, float]]:
        ...
        try:
            configs = self.db.query(RecommendationWeightConfig).all()
            for cfg in configs:
                rtype = cfg.recommendation_type.lower()
                if rtype in weights and cfg.metric_name in weights[rtype]:
                    weights[rtype][cfg.metric_name] = float(cfg.weight)
        except Exception:
            pass
        return weights
    ...
    def get_recommendations(self, quotation_id: int) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if upsell_candidates:
            ...
            for idx, c in enumerate(upsell_candidates, start=1):
                ...
                # Formula (Dynamic Weights):
                uw = self.weights["upsell"]
                final_score = int(round(
                    (uw["upgrade_frequency"] / 100.0) * norm_upgrade +
                    (uw["margin_opportunity"] / 100.0) * norm_margin +
                    (uw["promotion"] / 100.0) * norm_promo +
                    (uw["customer_affinity"] / 100.0) * norm_affinity +
                    (uw["stock_availability"] / 100.0) * norm_stock
                ))
        ...
        if cross_sell_candidates:
            ...
            for idx, c in enumerate(cross_sell_candidates, start=1):
                ...
                # Formula (Dynamic Weights):
                cw = self.weights["cross_sell"]
                final_score = int(round(
                    (cw["co_purchase_frequency"] / 100.0) * norm_copurchase +
                    (cw["compatibility"] / 100.0) * norm_compat +
                    (cw["promotion"] / 100.0) * norm_promo +
                    (cw["margin_opportunity"] / 100.0) * norm_margin +
                    (cw["stock_availability"] / 100.0) * norm_stock
                ))
        ...
        return {
            ...
            "weights": {
                "upsell": {k: int(v) if v.is_integer() else v for k, v in self.weights["upsell"].items()},
                "cross_sell": {k: int(v) if v.is_integer() else v for k, v in self.weights["cross_sell"].items()},
            }
        }
